...
class TestMazeWallCoordinates:
    ...
    def test_maze_walls_33_points(self):
        from frontend.pages.maze_animation_page import _MAZE_WALLS_X
...
class TestBuildCanvasPayloadHypothesis:
    ...
    @given(
        arrays(
            ...
            st.integers(5, 100),
            elements=st.one_of(
                st.floats(min_value=-1, max_value=8, allow_nan=False, allow_infinity=False),
                st.just(float("nan")),
            ),
        )
    )
...
class TestDrawMaze:
    ...
    def test_trace_has_correct_wall_data(self):
        import plotly.graph_objects as go
        from frontend.pages.maze_animation_page import _MAZE_WALLS_X, _MAZE_WALLS_Y, _draw_maze
