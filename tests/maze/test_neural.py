...
neural = pytest.importorskip("hm2p.maze.neural", reason="hm2p.maze.neural not yet implemented")
...
class TestNodeActivityMap:
    ...
    def test_shape(self, maze, rng):
        act, _ = self._call(
            _sig(rng, 5, 100), _walk(maze, rng, 100), np.ones(100, dtype=bool), maze.n_cells
        )
        assert act.shape == (5, maze.n_cells)

    def test_nan_unvisited(self, maze, rng):
        act, _ = self._call(
            _sig(rng, 3, 50), np.zeros(50, dtype=np.int32), np.ones(50, dtype=bool), maze.n_cells
        )
    ...
    def test_1d(self, maze, rng):
        act, _ = self._call(
            rng.standard_normal(50), _walk(maze, rng, 50), np.ones(50, dtype=bool), maze.n_cells
        )
        assert act.shape == (1, maze.n_cells)

    def test_empty_mask(self, maze, rng):
        act, _ = self._call(
            _sig(rng, 2, 30), _walk(maze, rng, 30), np.zeros(30, dtype=bool), maze.n_cells
        )
        assert np.all(np.isnan(act))
    ...
    def test_empty(self, maze):
        act, _ = self._call(
            np.empty((2, 0)), np.array([], dtype=np.int32), np.array([], dtype=bool), maze.n_cells
        )
        assert np.all(np.isnan(act))
    ...
    def test_prop_nan(self, n, s):
        ...
        if valid.any():
            np.add.at(occ, ci[valid], 1)
        for c in range(m.n_cells):
            if occ[c] == 0:
                assert np.all(np.isnan(act[:, c]))
            else:
                assert np.all(np.isfinite(act[:, c]))
...
class TestLightDarkNodeContrast:
    ...
    def test_min_frames_nan(self, maze, rng):
        ...
        lo = np.zeros(40, dtype=bool)
        lo[:10] = True
        d, _, _, _ = self._call(
            _sig(rng, 2, 40), ci, lo, np.ones(40, dtype=bool), maze.n_cells, min_frames=20
        )
        assert np.all(np.isnan(d[:, 0]))

    def test_min_frames_met(self, maze, rng):
        ci = np.zeros(100, dtype=np.int32)
        lo = np.zeros(100, dtype=bool)
        lo[:50] = True
        d, _, _, _ = self._call(
            _sig(rng, 2, 100), ci, lo, np.ones(100, dtype=bool), maze.n_cells, min_frames=20
        )
...
class TestLightModulationByNodeType:
    def test_keys(self, maze, rng):
        r = neural.light_modulation_by_node_type(
            ...
            np.ones(maze.n_cells, dtype=bool),
            maze,
    ...
    def test_mean_delta_shape(self, maze, rng):
        nr = 8
        r = neural.light_modulation_by_node_type(
            rng.standard_normal((nr, maze.n_cells)), np.ones(maze.n_cells, dtype=bool), maze
        )
        for nt in r:
    ...
    def test_wilcoxon_range(self, maze, rng):
        r = neural.light_modulation_by_node_type(
            rng.standard_normal((10, maze.n_cells)), np.ones(maze.n_cells, dtype=bool), maze
        )
        for nt in r:
            p = r[nt]["wilcoxon_p"]
            if np.isfinite(p):
    ...
    def test_single_ct(self, maze, rng):
        ...
        for nt in r:
            assert np.isnan(r[nt]["mannwhitney_p"])

    def test_no_valid(self, maze, rng):
        r = neural.light_modulation_by_node_type(
            rng.standard_normal((4, maze.n_cells)), np.zeros(maze.n_cells, dtype=bool), maze
        )
        for nt in r:
            assert np.all(np.isnan(r[nt]["mean_delta"]))

    def test_no_labels(self, maze, rng):
        r = neural.light_modulation_by_node_type(
            rng.standard_normal((6, maze.n_cells)), np.ones(maze.n_cells, dtype=bool), maze
        )
        for nt in r:
            assert "mannwhitney_U" not in r[nt]

    def test_few_rois(self, maze, rng):
        r = neural.light_modulation_by_node_type(
            rng.standard_normal((4, maze.n_cells)), np.ones(maze.n_cells, dtype=bool), maze
        )
        for nt in r:
            assert np.isnan(r[nt]["wilcoxon_p"])
    ...
    def test_wilcoxon_scipy(self, maze):
        ...
        r = neural.light_modulation_by_node_type(delta, np.ones(maze.n_cells, dtype=bool), maze)
        for nt in r:
            md = r[nt]["mean_delta"]
            f = np.isfinite(md)
            if f.sum() >= 6:
...
class TestClassifyFramesByNodeType:
    def test_keys(self, maze, rng):
        assert set(neural.classify_frames_by_node_type(_walk(maze, rng, 100), maze)) == {
            "junction",
            "corridor",
            "dead_end",
            "invalid",
    ...
    def test_oor(self, maze):
        r = neural.classify_frames_by_node_type(np.array([maze.n_cells], dtype=np.int32), maze)
        assert r["invalid"][0]
...
class TestJunctionVsCorridorMvl:
    ...
    def test_range(self, maze, rng):
        ...
        for k in ("junction_mvl", "corridor_mvl"):
            ...
            if len(f):
                assert np.all(f >= 0) and np.all(f <= 1 + 1e-6)
...
class TestCountCorridorTraversals:
    ...
    def test_non_corr(self, maze):
        ci = np.array(
            [maze.cell_to_idx[maze.junctions[0]], maze.cell_to_idx[maze.dead_ends[0]]],
            dtype=np.int32,
        )
...
class TestActivityByTraversalNumber:
    def test_keys(self, maze, rng):
        ...
        r = neural.activity_by_traversal_number(
            rng.standard_normal(n),
            ci,
            neural.count_corridor_traversals(ci, maze),
            np.ones(n, dtype=bool),
            5,
        )
    ...
    def test_spearman_range(self, maze, rng):
        ...
        if np.isfinite(r["spearman_r"]):
            assert -1 <= r["spearman_r"] <= 1

    def test_few_nan(self, maze):
        ci = np.full(10, maze.cell_to_idx[maze.corridors[0]], dtype=np.int32)
        r = neural.activity_by_traversal_number(
            np.ones(10), ci, neural.count_corridor_traversals(ci, maze), np.ones(10, dtype=bool), 5
        )
...
class TestExtractJunctionEvents:
    ...
    def test_dead_end_only(self, maze):
        assert (
            len(
                neural.extract_junction_events(
                    np.full(30, maze.cell_to_idx[maze.dead_ends[0]], dtype=np.int32), maze
                )
            )
            == 0
        )
...
class TestDecodeJunctionChoice:
    ...
    def test_single_class(self, rng):
        r = neural.decode_junction_choice(rng.standard_normal((20, 5)), np.zeros(20, dtype=int), 5)
        assert np.isnan(r["accuracy"])
...
class TestIntegration:
    def test_analysis4(self, maze, rng):
        ...
        if isinstance(cr, dict):
            ...
            delta = cr[0]
            vc = np.any(np.isfinite(delta), axis=0)
    ...
    def test_analysis1(self, maze, rng):
        ...
        r = neural.hd_tuning_by_location_type(
            rng.standard_normal(n), _hd(rng, n), loc, np.ones(n, dtype=bool), min_frames=10
        )
        for _, (tc, _, _) in r.items():
            assert tc.shape == (36,)
...
class TestEdgeCases:
    ...
    def test_single_frame(self, maze, rng):
        assert self._act(
            _sig(rng, 2, 1), np.array([0], dtype=np.int32), np.array([True]), maze.n_cells
        ).shape == (2, maze.n_cells)

    def test_nan_sig(self, maze, rng):
        n = 50
        ci = _walk(maze, rng, n)
        act = self._act(np.full((3, n), np.nan), ci, np.ones(n, dtype=bool), maze.n_cells)
        r = neural.node_activity_map(
            np.full((3, n), np.nan), ci, np.ones(n, dtype=bool), maze.n_cells
        )
    ...
    def test_large(self, maze, rng):
        ...
        r = neural.node_activity_map(
            np.full((2, n), 1e15), ci, np.ones(n, dtype=bool), maze.n_cells
        )
        occ = r[1] if isinstance(r, tuple) else None
        if occ is not None:
            for c in range(maze.n_cells):
    ...
    def test_negative(self, maze, rng):
        n = 80
        ci = _walk(maze, rng, n)
        sig = -np.abs(rng.standard_normal((3, n)))
        ...
        if occ is not None:
            for c in range(maze.n_cells):
                if occ[c] > 0:
                    assert np.all(act[:, c] < 0)

    def test_inf(self, maze, rng):
        n = 50
        sig = rng.standard_normal((2, n))
        sig[0, 10] = np.inf
        assert self._act(sig, _walk(maze, rng, n), np.ones(n, dtype=bool), maze.n_cells).shape == (
            2,
            maze.n_cells,
        )
