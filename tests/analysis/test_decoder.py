"""Tests for hm2p.analysis.decoder — Bayesian population HD decoder."""
...
class TestBuildDecoder:
    ...
    def test_tuning_curves_positive(self):
        """Tuning curves should be strictly positive (min_rate floor)."""
        signals, hd, mask = _make_population()
        dec = build_decoder(signals, hd, mask)
        assert np.all(dec["tuning_curves"] > 0)
...
class TestDecodeHD:
    ...
    def test_time_binning(self):
        ...
        dec = build_decoder(signals, hd, mask)
        decoded, posterior = decode_hd(signals, dec, time_bins=5)
        assert decoded.shape == (20,)
        assert posterior.shape == (20, 36)
    ...
    def test_good_population_decodes_well(self):
        ...
        # Mean absolute error should be < 45° with 20 well-tuned cells
...
class TestCrossValidatedDecode:
    ...
    def test_cv_better_than_chance(self):
        """CV decoding with tuned population should beat chance (90°)."""
        signals, hd, mask = _make_population(n_cells=15, kappa=4.0, noise=0.1)
        result = cross_validated_decode(
            signals, hd, mask, n_folds=5, rng=np.random.default_rng(42),
        )
        # Chance level for uniform distribution is ~90° MAE
