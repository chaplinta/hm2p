...
**Body parts tracked:** `nose_tip`, `left_ear`, `right_ear`, `implant_centre`, `neck`, `mid_back`, `mouse_center`, `tail_base`. All except `implant_centre` map to SuperAnimal TopViewMouse keypoints; `implant_centre` is a custom keypoint trained from scratch (high-contrast 2P headstage, easy to detect).
