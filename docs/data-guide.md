...
**Experiment type:** Freely-moving mouse in a q-rose maze (also open field and linear
...
Out-of-bounds positions are clipped to the nearest point inside the q-rose maze boundary
...
This polygon encodes the q-rose maze shape (a 7 × 5 unit grid with corridors). Positions
...
| `exp_id` | str | Session ID (`YYYYMMDD_HH_MM_SS_<animal_id>`) — all sessions are q-rose maze type |
