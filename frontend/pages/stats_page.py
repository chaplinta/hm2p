...
sessions = session_filter_sidebar(
    all_sessions, show_roi_filter=True, key_prefix="stats"
)
...
n_penk = len(penk_animals)
n_nonpenk = len(nonpenk_animals)
...
if len(penk_maxdff) > 0 and len(nonpenk_maxdff) > 0:
    ...
    tests_results.append({
        "Test": "Max dF/F0: Penk vs Non-Penk",
        "Statistic": f"U = {stat:.0f}",
        "p-value": f"{pval:.4f}",
        "Significant": "Yes" if pval < 0.05 else "No",
